...
// Standard template for the digital forms
const generateTemplate = (formName: string, formId: string) => `
'use client';

import React from 'react';
import { FileText, Save, Printer, ArrowLeft } from 'lucide-react';

export default function DigitalForm_${formId.replace(/[^a-zA-Z0-9]/g, '_')}() {
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-20">
      <div className="flex items-center justify-between bg-white/5 border border-white/10 p-6 rounded-3xl backdrop-blur-xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">${formName.toUpperCase()}</h1>
            <p className="text-slate-400 text-xs font-medium">${formId}</p>
          </div>
        </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-3xl backdrop-blur-xl p-8 min-h-[400px]">
        <div className="flex flex-col items-center justify-center h-full py-20 text-center">
          ...
          <p className="text-slate-500 max-w-md mx-auto text-sm">
            Это черновик цифровой формы ${formId}. Вы можете отредактировать этот файл в src/components/forms/DigitalForm_${formId.replace(/[^a-zA-Z0-9]/g, '_')}.tsx
          </p>
...
const INDEX_PATH = path.join(process.cwd(), 'src/data/regulatory_index.json');
const COMPONENTS_DIR = path.join(process.cwd(), 'src/components/forms');
...
async function scaffold() {
  ...
  index.appendices.forEach((app: any) => {
    app.forms.forEach((form: any) => {
      // Improve slugification: "Форма_1" -> "1", "Форма_26_часть_2" -> "26_part_2"
      let slug = form.id
        .replace(/^Форма_/, '')
        .replace(/_часть_/, '_part_')
        .replace(/[^a-zA-Z0-9_]/g, '');
      
      const fileName = `DigitalForm${slug}.tsx`;
      const filePath = path.join(COMPONENTS_DIR, fileName);

      if (!fs.existsSync(filePath)) {
        console.log(`Generating ${fileName}...`);
        fs.writeFileSync(filePath, generateTemplate(form.name, form.id));
        count++;
      }
    });
  });

  console.log(`Successfully scaffolded ${count} new forms.`);
}
